...
import { MOCK_STORES, MOCK_INSPECTIONS, MOCK_INSPECTOR } from '../mock';
import { INSPECTION_CATEGORIES, getScoreLevel, getMaxTotalScore } from '../constants';

type DataChangeListener = (type: 'inspection' | 'audit' | 'store') => void;

class InspectionService {
  private inspections: Inspection[] = [...MOCK_INSPECTIONS];
  ...
  private notify(type: 'inspection' | 'audit' | 'store') {
  ...
  canSubmitInspection(storeId: string, date: string): { allowed: boolean; reason?: string; existingVersion?: number } {
    const existing = this.getInspectionsByStoreAndDate(storeId, date).filter(
  ...
  submitInspection(data: InspectionSubmissionData): { success: boolean; message: string; auditRecord?: AuditRecord } {
  ...
  private getNextVersion(inspectionId: string): number {
    const records = this.auditRecords.filter(
      (r) => r.inspectionId === inspectionId && (r.action === 'submit' || r.action === 'resubmit')
    );
    return records.length + 1;
  }

  private updateStoreAverageScore(storeId: string) {
    const storeInspections = this.inspections.filter(
      (i) => i.storeId === storeId && i.status === 'completed'
    );
  ...
  getStoreRankings(): StoreRanking[] {
    ...
    return sortedStores.map((store, index) => {
      ...
      if (index < prevRanking) trend = 'up';
      else if (index > prevRanking) trend = 'down';
  ...
  getMonthlyReport(month: string): MonthlyReport {
    ...
    const areaMap = new Map<string, { stores: Set<string>; inspections: Inspection[] }>();
    ...
    const areaStatistics: AreaStatistic[] = [];
  ...
  downloadReport(month: string, format: 'csv' | 'pdf' = 'csv') {
    if (format === 'csv') {
