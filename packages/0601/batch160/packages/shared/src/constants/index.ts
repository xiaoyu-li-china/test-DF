import { InspectionItem, InspectionCategory } from '../types';
...
export const getMaxTotalScore = (): number => {
  return INSPECTION_CATEGORIES.reduce((total, category) => {
    return total + category.items.reduce((sum, item) => sum + item.maxScore, 0);
  }, 0);
};

export const getScoreLevel = (score: number, maxScore: number): 'excellent' | 'good' | 'average' | 'poor' => {
