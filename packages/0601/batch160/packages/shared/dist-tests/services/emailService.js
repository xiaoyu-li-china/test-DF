...
class EmailService {
    ...
    initMockSchedules() {
        this.schedules.push({
            ...
            subject: '月度门店巡检报告',
            enabled: true,
            nextSendAt: (0, dayjs_1.default)().add(1, 'minute').toISOString(),
            ...
            subject: '周度门店巡检数据汇总',
            enabled: true,
            nextSendAt: (0, dayjs_1.default)().add(2, 'day').toISOString(),
            createdAt: new Date().toISOString(),
            createdBy: 'admin001',
        });
    }
    ...
    async sendScheduleEmail(schedule) {
        ...
        const reportMonth = (0, dayjs_1.default)().subtract(1, 'month').format('YYYY-MM');
        const report = inspectionService_1.inspectionService.getMonthlyReport(reportMonth);
        const log = {
            id: (0, utils_1.generateId)(),
    ...
    sendTestEmail(recipient, month) {
        const reportMonth = month || (0, dayjs_1.default)().format('YYYY-MM');
        const report = inspectionService_1.inspectionService.getMonthlyReport(reportMonth);
    ...
    getReportEmailContent(month) {
        const report = inspectionService_1.inspectionService.getMonthlyReport(month);
      ...
      <body>
        <div class="header">
          ...
          <p>自动生成于 ${(0, utils_1.formatDate)(new Date().toISOString(), 'YYYY年MM月DD日 HH:mm')}</p>
        </div>
